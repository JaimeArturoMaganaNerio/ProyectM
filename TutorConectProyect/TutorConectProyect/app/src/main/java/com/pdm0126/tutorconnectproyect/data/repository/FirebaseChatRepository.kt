...
class FirebaseChatRepository @Inject constructor(
    ...
    override fun getConversations(userId: String): Flow<Resource<List<Conversation>>> = callbackFlow {
        trySend(Resource.Loading)
        val listener = firestore.collection("messages")
            .orderBy("timestamp", Query.Direction.DESCENDING)
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    trySend(Resource.Error(error.message ?: "Error cargando conversaciones"))
                    return@addSnapshotListener
                }
                val allMessages = snapshot?.toObjects(ChatMessage::class.java) ?: emptyList()
                // Mensajes donde el usuario es sender o receiver
                val myMessages = allMessages.filter {
                    it.senderId == userId || it.receiverId == userId
                }
                // Agrupar por el otro participante, tomando el más reciente de cada par
                val conversationMap = mutableMapOf<String, ChatMessage>()
                ...
                // Convertir a Conversation (nombre se resuelve en el ViewModel)
                val conversations = conversationMap.map { (otherId, lastMsg) ->
                    Conversation(
                        otherUserId = otherId,
                        otherUserName = otherId, // placeholder; se reemplaza en el ViewModel
                        lastMessage = lastMsg.message,
                        lastMessageTimestamp = lastMsg.timestamp.time,
                    )
